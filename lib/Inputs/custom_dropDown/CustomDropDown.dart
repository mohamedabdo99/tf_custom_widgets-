...
class DropdownSearchState<T> extends State<DropdownSearch<T>> {
  ...
  SelectDialog<T> _selectDialogInstance(T? data, {double? defaultHeight}) {
    return SelectDialog<T>(
      searchBoxStyle: widget.searchFieldProps?.style,
      ...
      selectedValue: data,
      searchBoxDecoration: widget.searchFieldProps?.decoration,
      ...
      autoFocusSearchBox: widget.searchFieldProps?.autofocus ?? false,
      dialogMaxWidth: widget.dialogMaxWidth,
      itemDisabled: widget.popupItemDisabled,
      searchBoxController: widget.searchFieldProps?.controller ?? TextEditingController(),
